...
function renderPhotos() {
  ...
  card.innerHTML = `
    ...
    <p class="step-sub">Upload a photo and it's automatically logged under today's date (${todayStr()}). Saved on the server — refreshing or closing the app won't lose it.</p>
  `;
  stepContainer.appendChild(card);
  ...
  uploadInput.addEventListener('change', async () => {
    ...
    try {
      ...
      const res = await fetch('/api/photos', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ date: todayStr(), dataUrl }),
      });
      if (!res.ok) throw new Error('upload failed');
      statusMsg.style.display = 'none';
      await renderPhotoGallery(galleryContainer);
    } catch (e) {
...
async function renderPhotoGallery(container) {
  ...
  try {
    ...
    container.innerHTML = '<p class="empty-hint">Could not load photos.</p>';
    return;
  }
  container.innerHTML = '';
  if (photos.length === 0) {
